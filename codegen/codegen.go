...
func call(up chan assembly, ast lexparse.Ast, counter func() int, sym *safeSym) {
        ...
        if isFunc {
                ...
                up <- assembly{"SETI", r3, 1, Type_closure}
                up <- assembly{"SETI", r3, 2, funcStart}
                up <- assembly{"SETID", r3, 3, r2}
                up <- assembly{"SETID", r3, 4, args+5}
                for i := 0; i < args; i++ {
                ...
                up <- assembly{"SETD", r3, r2, 7}       // grab address with ptr to func symbol
                up <- assembly{"DEREF", r3, r3, 0}       // grab address of func sym
                up <- assembly{"SETD", r3, r3, 2}       // grab address of sym id
                up <- assembly{"DEREF", r3, r3, 0}       // grab sym id
                ...
                for m := 1; m < members; m++ {
                        ...
                        up <- assembly{"SETI", r5, 0, 1}
                        up <- assembly{"SETI", r5, 1, Type_symtab}
                        up <- assembly{"SETD", r3, r4, 4}       // grab length of closure
                        up <- assembly{"SUB", r3, 4, 0}         // subtract 4 from length to get number of args
                        up <- assembly{"SET", r7, 0, 0}
                        loop = counter()
                        end = counter()
                        up <- assembly{"LABEL", loop, 0, 0}
